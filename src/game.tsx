...
function RacingGame() {
  ...
  useEffect(() => {
    ...
    const tick = (dt: number) => {
      ...
      camera.position.lerp(new THREE.Vector3(state.x + curveXAtS(state.z), 2.5, -5), paramsNow.camFollowLerp);

      const lookAheadZ = 3;
      const lookS = state.z + 8;
      const lookPos = new THREE.Vector3(curveXAtS(lookS), 0.5, lookAheadZ);
